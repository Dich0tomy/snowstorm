...
myawesomemenu = {
   ...
   { "EDIT_CONFIG", editor_cmd .. " " .. awesome.conffile },
...
globalkeys = gears.table.join(globalkeys,
    ...
    awful.key(
      { "Ctrl" },
      "Print",
      bandit.window_screenshot,
      {
        description = "Take a screenshot of active window",
        group = "screenshot"
      }
    ),
    ...
    awful.key(
      { modkey, "Shift" },
      "s",
      bandit.selection_screenshot,
      {
        description = "Take a screenshot with selection",
        group = "screenshot"
      }
    ),
  ...
  awful.key(
    { 'Mod1' },
    'Shift_L',
    function() Polyglot:set_next() end,
    {
      description = ('Set next keyboard layouts [%s]'):format(table.concat(layouts, ', ')),
      group = 'layouts'
    }
  ),
...
-- Yoinked from https://www.reddit.com/r/awesomewm/comments/d8r74k/detecting_spotify/
client.connect_signal(
  "property::class",
  function(c)
    local chuck_to = function(name, tag)
      if c.class == name then
        -- Check if Spotify is already open
        local spotify = function (c)
          return awful.rules.match(c, { class = name })
        end
        ...
        if spotify_count > 1 then
          ...
          local t = awful.tag.find_by_name(awful.screen.focused(), " " .. tag .. " ")
          c:move_to_tag(t)
        end
      end
    end
